...
import org.firstinspires.ftc.teamcode.RelicRecovery.Sensors.Experiments.IMU_Tester_3;

/**
 * Created by Jorge on 23/11/2017.
 */
@Autonomous(name="Tester_Autonomous")
public class Tester_Autonomous extends LinearOpMode {
...
*/
     ...
     /*   relicTrackables.activate();
        ...
        if (vuMark == RelicRecoveryVuMark.LEFT) {
            ...
            Multi = "UNKNOWN - CENTER";
        }
        telemetry.addData("Multi: ", Multi);
        telemetry.update();
        sleep(5000);*/
        ImuTurnBlue(.4,-110,2);
        sleep(1000);
    ...
    protected void RightWheelsByRotations(double power, double rotations){
        ...
        R_M_F.setPower(power);
        R_M_B.setPower(power);
        while ((Math.abs(R_M_B.getCurrentPosition()) < Target && Math.abs(R_M_F.getCurrentPosition()) < Target)){
            R_M_F.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            R_M_B.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
            FourEncodersTelemetry();
            telemetry.update();
    ...
    public void HitJewel(double power, double rotations){
        if (StoneColor == red) {
            ...
            if (JewelColor == red) {
                ...
                StopDriving();
                sleep(200);
                RightWheelsByRotations(power, rotations+.1);
            } else if (JewelColor == blue) {
                RightWheelsByRotations(power, rotations);
                R_S_J.setPosition(.1);
                StopDriving();
                sleep(200);
                RightWheelsBackByRotations(power, rotations+.1);
            } else {
            ...
            if (JewelColor == red) {
                ...
                L_S_J.setPosition(1);
                LeftWheelsBackByRotations(power, rotations+.1);//Without +.1
            } else if (JewelColor == blue) {
